...
def render_sidebar():
    ...
    for group_title, group_icon, items in nav_groups:
        # Role-based visibility: only keep pages this role is allowed to see.
        visible_items = [(label, icon, view_key) for label, icon, view_key in items if role_can_access(role, view_key)]
        if not visible_items:
            continue  # whole chapter has nothing relevant for this role — hide it
        is_active_group = any(view_key == current_view for _, _, view_key in visible_items)
        with st.sidebar.expander(f"{group_icon} {group_title}", expanded=is_active_group):
            for label, icon, view_key in visible_items:
                nav_button(label, icon, view_key)
    ...
    if role in ("boss", "vendor"):
        ...
        is_admin_active = any(view_key == current_view for _, _, view_key in admin_items)
        with st.sidebar.expander("6\uFE0F\u20E3 \U0001F451 Boss / Vendor Tools", expanded=is_admin_active):
            for label, icon, view_key in admin_items:
                nav_button(label, icon, view_key)
...
@keyframes ctaPulse { 0%,100% { transform:scale(1); box-shadow:0 4px 14px rgba(255,215,0,0.4); } 50% { transform:scale(1.03); box-shadow:0 8px 26px rgba(255,215,0,0.8); } }""")
    ...
    with tab_chat:
        ...
        with chat_col2:
            ...
            with top_bar_r:
                if db_connected and st.button("\U0001F504 Refresh", key=f"chat_refresh_{contact_name}", use_container_width=True):
...
.airec-card { background: white; padding: 18px 20px; border-radius: 14px; box-shadow: 0 6px 15px rgba(0,0,0,0.06); border-left: 5px solid #D4AF37; margin-bottom: 14px; }
.airec-score { font-weight: 900; color: #27AE60; font-size: 1.2rem; }""")
    ...
    with st.expander("\U0001F39B\uFE0F Advanced Filters", expanded=False):
        ...
        with f1:
            age_range = st.slider("Age range", 21, 55, (26, 34))
            height_range = st.slider("Height range (cm)", 140, 200, (155, 180))
        with f2:
            income_range = st.select_slider("Annual income (\u20b9 Lakhs)", options=[3, 5, 8, 12, 18, 25, 40, "40+"], value=(5, 18))
            diet_pref = st.selectbox("Diet preference", ["Any", "Vegetarian", "Non-Vegetarian", "Eggetarian", "Vegan"])
        with f3:
            manglik_pref = st.selectbox("Manglik status", ["Any", "Manglik", "Non-Manglik", "Anshik Manglik"])
            habits_pref = st.selectbox("Drinking / Smoking", ["Any", "Non-drinker & Non-smoker only", "Occasional drinker OK", "No preference"])
        f4, f5 = st.columns(2)
        education_pref = f4.selectbox("Minimum education", ["Any", "Graduate", "Post-Graduate", "Doctorate"])
        location_pref = f5.text_input("Preferred city/region", placeholder="e.g., Mumbai, Pune")
        st.button("\U0001F50D Apply Filters & Refresh Recommendations", type="primary", use_container_width=True)

    st.markdown("### \u2728 Today's Top Picks for You")
    recs = [
        {"name": "Ritu Deshmukh", "age": 27, "city": "Pune", "score": 94, "why": "Shared values on family & career, similar education background."},
        {"name": "Sneha Patil", "age": 29, "city": "Mumbai", "score": 91, "why": "High Kundli compatibility and matching lifestyle preferences."},
        {"name": "Ananya Rao", "age": 26, "city": "Bengaluru", "score": 88, "why": "Compatible age gap, both prioritize fitness and travel."},
        {"name": "Kavya Nair", "age": 28, "city": "Chennai", "score": 85, "why": "Similar community background, overlapping interests in music."},
    ]
    for r in recs:
        st.markdown(f"""
        <div class="airec-card">
            <b style="font-size:1.1rem;">{r['name']}, {r['age']}</b> &middot; {r['city']}
            <span class="airec-score" style="float:right;">{r['score']}% Match</span><br>
            <span style="color:gray; font-size:0.9rem;">{r['why']}</span>
        </div>
        """, unsafe_allow_html=True)
        rc1, rc2, rc3 = st.columns(3)
        rc1.button("\U0001F49B Interested", key=f"airec_like_{r['name']}", use_container_width=True)
        rc2.button("\U0001F441\uFE0F View Profile", key=f"airec_view_{r['name']}", use_container_width=True)
        rc3.button("\u274C Not Interested", key=f"airec_skip_{r['name']}", use_container_width=True)
