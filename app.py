...
def show_profile_detail_modal(r):
    ...
    used by every 'View Profile' button across the app (Search Partner,
    My Matches, AI Smart Recommendations) instead of each page having its
    own small inline expander. Works with any of the reshaped profile
    dicts (real or demo) since they share the same common keys."""
    ...
    with dcol2:
        st.markdown(f"## {r.get('name','')}, {r.get('age','')}")
        ...
        detail_rows = [
            ...
            ("\U0001F468\u200D\U0001F469\u200D\U0001F467 Family", (f"{r.get('num_brothers',0)} brother(s), {r.get('num_sisters',0)} sister(s)" if (r.get("num_brothers") is not None) else None)),
        ]
...
def contains_contact_info(text):
    ...
    # 1. Raw digit sequences with common separators stripped (phone numbers)
    ...
    # 4. Explicit mentions of taking the conversation off-platform
            ...
            if not blocked_list:
                ...
                for b in blocked_list:
                    ...
                    if bcol2.button("Unblock", key=f"unblock_{b['blocked_email']}", use_container_width=True):
                        db_unblock_viewer(user_email, b["blocked_email"])
                        st.rerun()
            st.markdown("---")
        ...
        if not results:
            ...
            for idx, r in enumerate(results, start=1):
                ...
                if b5.button("\u2705 Interest Sent" if already_sent else "\U0001F49B Send Interest", key=f"meet_{r['_uid']}", use_container_width=True, disabled=already_sent):
                    ...
                    st.rerun()
                if r.get("is_real") and r.get("email"):
                    if b6.button("\U0001F6AB Block", key=f"block_{r['_uid']}", use_container_width=True, help="They will no longer be able to see your profile."):
                        db_block_viewer(searcher_email, r["email"])
                        st.success(f"\u2705 {r['name']} can no longer see your profile. Manage this anytime in Edit My Profile.")
                        st.rerun()
                st.markdown("<br>", unsafe_allow_html=True)
...
@keyframes ctaPulse { 0%,100% { transform:scale(1); box-shadow:0 4px 14px rgba(255,215,0,0.4); } 50% { transform:scale(1.03); box-shadow:0 8px 26px rgba(255,215,0,0.8); } }""")
    ...
    tab_chat, tab_alerts = st.tabs(["\U0001F4AC Secure In-App Messages", "\U0001F514 Match & Email Alerts"])
        ...
        with chat_col2:
            ...
            if pending_msg:
                ...
                if contains_contact_info(pending_msg):
                    st.error("\U0001F6AB For everyone's safety, phone numbers, email addresses, and requests to move the conversation off Bandhan.com aren't allowed in chat. Please continue the conversation here \u2014 numbers unlock automatically once you're both comfortable and a family meet is scheduled.")
                else:
...
.report-history-item { background: #F8F9FA; border-left: 4px solid #D4AF37; padding: 12px 15px; border-radius: 8px; margin-bottom: 10px; }""")
    ...
    with tab1:
        ...
        reported_email_input = st.text_input("Their registered email, if you know it (helps us actually reach them with a warning)", value=prefill_id if (prefill_id and "@" in str(prefill_id)) else "")
        ...
        if st.button("\U0001F6A9 Submit Report", type="primary", use_container_width=True):
            if profile_name:
                ...
                reported_email_clean = reported_email_input.strip() or (prefill_id if (prefill_id and "@" in str(prefill_id)) else None)
                if sync_ok:
                    ref_id = db_save_safety_report(user_email, profile_name, reason, details, evidence is not None, reported_email=reported_email_clean)
...
.sync-pill { display:inline-block; padding: 4px 12px; border-radius: 20px; font-size: 0.8rem; font-weight: 600; }""")
    ...
    # -----------------------------------------------------------------
    with st.expander("\U0001F514 Email Reminders for Pending Tasks"):
        ...
        opt_in = st.checkbox("Send me an Email nudge for tasks and unbooked services that are due soon", value=st.session_state.wc_reminder_opt_in)
        ...
        if st.button("\U0001F4E4 Send Reminder Now", key="wc_send_reminder"):
            ...
            if not reminder_email.strip():
                ...
                sent, detail = send_generic_email(reminder_email.strip(), "Your Wedding Countdown Reminder", reminder_message)
                if sent:
                    st.success(f"\u2705 Reminder emailed to {reminder_email}.")
                else:
                    st.info(f"\U0001F4E9 **Demo Mode:** Would email **{reminder_email}**:\n\n> {reminder_message}\n\n({detail})")
...
"""
        ...
        act1, act2, act6, act3, act4, act5 = st.columns(6)
        ...
        share_body = urllib.parse.quote(f"My Bandhan.com AI Relationship Forecast with {display_name}: {overall_score}% ({verdict})")
        act2.link_button("\U0001F4E7 Email", f"mailto:?subject={urllib.parse.quote('My Bandhan.com Relationship Forecast')}&body={share_body}", use_container_width=True)
        act6.link_button("\U0001F4F1 WhatsApp", f"https://wa.me/?text={share_body}", use_container_width=True)
    ...
    if not pending_reports:
        ...
        for rep in pending_reports:
            with st.container():
                ...
                if rep.get("reported_email"):
                    st.caption(f"\U0001F4E7 Reported person's email on file: {rep['reported_email']}")
                else:
                    st.caption("\u2139\uFE0F No email on file for the reported person \u2014 can't send an automatic warning email for this one.")
                rc1, rc2, rc3 = st.columns(3)
                if rc1.button("\u26A0\uFE0F Send Warning", key=f"warn_report_{rep['_id']}", use_container_width=True, disabled=not rep.get("reported_email")):
                    sent, info = db_send_safety_warning(str(rep["_id"]), rep.get("reported_email"))
                    if sent:
                        st.success(f"\u2705 Warning email sent to {rep['reported_email']}.")
                    else:
                        st.error(f"\u274C Couldn't send warning: {info}")
                    st.rerun()
                if rc2.button("\u2705 Mark Reviewed", key=f"review_report_{rep['_id']}", use_container_width=True):
                    db_update_safety_report_status(str(rep["_id"]), "Reviewed")
                    st.rerun()
                if rc3.button("\u274C Dismiss", key=f"dismiss_report_{rep['_id']}", use_container_width=True):
    ...
    if accepted:
        ...
        if not st.session_state.vcall_active:
            ...
            if app_id and agora_token:
                ...
                """)
                st.components.v1.html(
                    agora_video_widget_html(app_id, channel, agora_token, 0, st.session_state.get("user_name", "You")),
                    height=340,
                )
            else:
