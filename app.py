...
def db_list_recent_registered_profiles(limit=15):
    """Recently-registered members (name + photo) for the 'Newly
    Registered Members' showcase strip on Home / Search Partner /
    Registration. This is a public highlight reel, not the actual
    searchable listing — it shows photos immediately (require_approval
    off) the same way a person's own profile view does; the real Search
    Partner results still respect photo moderation status."""
    db = get_db()
    if db is None:
        return []
    docs = list(
...
def render_registered_showcase():
    """Auto-scrolling strip of newly registered members' names + photos.
    Shown on Home, Search Partner, and Registration pages. Silently does
    nothing if there aren't at least a couple of real registered profiles
    yet (e.g. demo mode / brand-new deployment)."""
    ...
    for p in profiles * 2:  # duplicated so the CSS scroll loop looks seamless
        ...
        city_html = f"<div style='font-size:0.72rem; color:#718096;'>{p['city']}</div>" if p["city"] else ""
        cards_html += f"""
        <div style="display:inline-block; text-align:center; margin:0 14px; width:110px; vertical-align:top;">
            <img src="{photo}" style="width:80px; height:80px; border-radius:50%; object-fit:cover; border:3px solid #D4AF37; box-shadow:0 4px 10px rgba(0,0,0,0.15);">
            <div style="font-weight:700; color:#1A365D; font-size:0.85rem; margin-top:6px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">{p['name']}</div>
            {city_html}
        </div>
        """
    ...
    <div style="margin-top:30px;">
        ...
        <div style="overflow:hidden; white-space:nowrap; padding:15px 0; background:linear-gradient(90deg, #FFF8E7, #FFFFFF, #FFF8E7); border-radius:14px;">
            <div style="display:inline-block; animation: scrollRegisteredProfiles 40s linear infinite;">
                {cards_html}
            </div>
        </div>
    </div>
    <style>
    @keyframes scrollRegisteredProfiles {{
