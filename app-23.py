...
def db_create_user(email, password_hash, role, name, gender=None):
    db = get_db()
    if db is None:
        return False
        ...
        if st.button("Create Account", type="primary", use_container_width=True):
            ...
            if not su_name_clean or not su_email_clean or not su_password_clean:
                ...
                if db_find_user(su_email_clean):
                    ...
                    created = db_create_user(su_email_clean, password_hash, role_value, su_name_clean, gender_value)
                    if created:
                        st.session_state.logged_in = True
...
.forecast-photo { width: 100%; border-radius: 12px; height: 160px; object-fit: cover; }""")
    ...
    if st.button("\U0001F52E Generate Unified Forecast", type="primary", use_container_width=True):
        ...
        # Deterministic-but-varied scoring per profile (stand-in until this
        # is wired to a real Kundli engine + quiz answers in a later step).
        seed = abs(hash(name_pick)) % 1000
        guna_score = 22 + (seed % 13)  # 22-34 out of 36
        quiz_match = 78 + (seed % 19)  # 78-96%
        career_match = 72 + (seed % 24)  # 72-95%
        family_options = [
        ...
        factors = [
            ...
            ("\U0001F9E9 Values & Lifestyle Quiz", f"{quiz_match}% match", "Both prioritize family, similar social energy levels." if quiz_match >= 85 else "Some differing lifestyle preferences worth discussing."),
            ("\U0001F3E1 Family Background", family_status, family_note),
            ("\U0001F4BC Career & Ambition", f"{career_match}% match", "Both career-focused with balanced approach to family time." if career_match >= 80 else "Different career priorities \u2014 balance may need conversation."),
        ]
...
"""
        ...
        if act2.button("\U0001F4AC Message", key=f"forecast_msg_{name_pick}", use_container_width=True):
        ...
        if act3.button("\u2B50 Saved" if is_shortlisted else "\u2606 Shortlist", key=f"forecast_short_{name_pick}", use_container_width=True):
            if is_shortlisted:
        ...
        if act4.button("\u2705 Sent" if already_interested else "\U0001F49B Send Interest", key=f"forecast_int_{name_pick}", use_container_width=True, disabled=already_interested):
...
.ice-photo { width: 100%; border-radius: 12px; height: 150px; object-fit: cover; }""")
    ...
    if st.button("\u2728 Generate Icebreakers", type="primary", use_container_width=True):
        ...
        st.session_state["_icebreaker_suggestions"] = suggestions
    ...
    if suggestions and suggestions_for == match_pick:
        for i, s in enumerate(suggestions):
            ...
            if c2.button("Send", key=f"use_{match_pick}_{i}", use_container_width=True):
                st.session_state.chat_preselect_contact = match_pick
                st.session_state.chat_pending_message = s
                go_to("chat_alerts")
            if c3.button("\U0001F4CB Copy", key=f"copy_{match_pick}_{i}", use_container_width=True):
                st.toast("\U0001F4CB Select and copy the text above \u2014 clipboard access isn't available in-app.")
...
.boost-expired-badge { background: #FEF3C7; color: #92400E; padding: 12px 20px; border-radius: 10px; font-weight: 700; text-align: center; margin-top: 15px; }""")
    ...
    if "client_boost_active" not in st.session_state:
        st.session_state.client_boost_active = None  # {"label":..., "expires_at": datetime}
    if "vendor_boost_active" not in st.session_state:
        st.session_state.vendor_boost_active = None  # {"name":..., "label":..., "expires_at": datetime}
    ...
    def is_still_active(boost):
        return boost is not None and dt.datetime.now() < boost["expires_at"]
    ...
    with tab1:
        ...
        client_active = st.session_state.client_boost_active
        ...
        for col, plan in zip(cols, plans):
            with col:
                ...
                if st.button(f"Activate {plan['label']} Boost", key=f"client_boost_{plan['label']}", use_container_width=True, disabled=is_this_active):
                    ...
                    st.session_state.boost_history.append({"type": "Profile", "label": plan["label"], "price": plan["price"], "activated_at": dt.datetime.now().strftime("%d %b %Y, %I:%M %p")})
                    st.rerun()

        if client_is_live:
            render_html(f"<div class='boost-active-badge'>\u2705 Your profile boost is active for {client_active['label']} \u2014 {time_left_str(client_active['expires_at'])}. You'll appear at the top of search results.</div>")
            st.caption("Demo mode: this simulates activation. Real payment integration requires a payment gateway (Razorpay/Stripe).")
            if st.button("\U0001F5D1\uFE0F Remove Boost", key="remove_client_boost"):
    ...
    with tab2:
        ...
        vendor_active = st.session_state.vendor_boost_active
        vendor_is_live = is_still_active(vendor_active)
        if vendor_active and not vendor_is_live:
            st.info(f"\u23F0 {vendor_active['name']}'s previous {vendor_active['label']} boost has expired.")
            ...
            with col:
                ...
                if st.button(f"Activate {plan['label']} Vendor Boost", key=f"vendor_boost_{plan['label']}", use_container_width=True, disabled=is_this_active):
                    if vendor_name:
                        ...
                        st.session_state.boost_history.append({"type": "Vendor", "label": plan["label"], "price": plan["price"], "activated_at": dt.datetime.now().strftime("%d %b %Y, %I:%M %p"), "vendor": vendor_name})
                        st.rerun()
                    else:
                        st.warning("Please enter your registered business name first.")

        if vendor_is_live:
            render_html(f"<div class='boost-active-badge'>\u2705 {vendor_active['name']}'s listing is boosted for {vendor_active['label']} \u2014 {time_left_str(vendor_active['expires_at'])}. It's featured at the top of its category.</div>")
            st.caption("Demo mode: this simulates activation. Real payment integration requires a payment gateway (Razorpay/Stripe).")
            if st.button("\U0001F5D1\uFE0F Remove Vendor Boost", key="remove_vendor_boost"):
                st.session_state.vendor_boost_active = None
                st.rerun()

    if st.session_state.boost_history:
        st.markdown("---")
        st.markdown("### \U0001F4CB Your Boost History")
        hist_df = pd.DataFrame(st.session_state.boost_history)
...
.grievance-box { background: #FFF8E1; border-left: 5px solid #D4AF37; padding: 18px; border-radius: 8px; margin-top: 15px; }""")
    ...
    with tab4:
        ...
        dpdp_email = st.text_input("Your registered Email / Mobile Number", key="dpdp_req_email")
        ...
        if st.button("\U0001F4E4 Submit Data Rights Request", type="primary", use_container_width=True, key="dpdp_req_submit"):
            if dpdp_email and confirm_check:
                ...
                if db_is_connected():
                    db_save_dpdp_request(dpdp_email, clean_type, dpdp_details)
                st.success(f"\u2705 Your **{clean_type}** request has been received. Our Grievance Officer will respond within the statutory timeline (typically within 30 days, sooner for straightforward requests).")
            else:
...
.vidpro-badge { background: #DCFCE7; color: #166534; padding: 8px 16px; border-radius: 10px; font-weight: 700; display:inline-block; }""")
    ...
    with tab1:
        st.caption("Tip: introduce yourself, your interests, and what you're looking for in a partner.")
        cam_video = st.camera_input("Record a short intro (webcam)", key="vidpro_camera")
        if cam_video is not None:
            st.session_state.profile_video_bytes = cam_video.getvalue()
            st.session_state.profile_video_status = "Pending Review"
            st.success("\u2705 Intro clip captured! It will appear on your profile after moderation.")
    with tab2:
        uploaded_video = st.file_uploader("Upload MP4/MOV (max 60 seconds recommended)", type=["mp4", "mov", "m4v"], key="vidpro_upload")
        if uploaded_video is not None:
            st.session_state.profile_video_bytes = uploaded_video.getvalue()
            st.session_state.profile_video_status = "Pending Review"
            st.success("\u2705 Video uploaded! It will appear on your profile after moderation.")
    st.markdown("</div>", unsafe_allow_html=True)
...
.vcall-room { background: #0F2027; border: 2px dashed #D4AF37; border-radius: 16px; padding: 40px; text-align: center; color: #E2E8F0; }""")
    ...
    match_choice = st.selectbox("Choose a mutual match", ["Priya Sharma", "Aisha Khan", "Ritu Deshmukh", "Sneha Patil"])
    c1, c2 = st.columns(2)
    call_date = c1.date_input("Preferred date")
    call_time = c2.time_input("Preferred time")
    if st.button("\U0001F4E4 Send Call Request", type="primary", use_container_width=True):
    ...
    if st.session_state.vcall_requests:
        ...
        for i, req in enumerate(st.session_state.vcall_requests):
            ...
            rc1.write(f"**{req['with']}** — {req['date']} at {req['time']} — Status: *{req['status']}*")
            if req["status"] == "Pending" and rc2.button("Simulate Accept", key=f"vcall_accept_{i}"):
    ...
    if accepted:
        ...
        joinable = st.selectbox("Accepted calls", [f"{r['with']} — {r['date']} {r['time']}" for r in accepted])
        if not st.session_state.vcall_active:
            if st.button("\U0001F7E2 Join Call Room", type="primary", use_container_width=True):
                st.session_state.vcall_active = True
                st.rerun()
        else:
            render_html(f"""
            <div class="vcall-room">
                <h2>\U0001F534 LIVE — In call with {joinable.split(' — ')[0]}</h2>
...
.notifpref-card { background: white; padding: 22px; border-radius: 14px; box-shadow: 0 6px 15px rgba(0,0,0,0.06); border: 1px solid #EAEAEA; }""")
    ...
    c1, c2, c3 = st.columns(3)
    push_on = c1.checkbox("\U0001F4F1 Push Notifications", value=True, key="notif_channel_push")
    email_on = c2.checkbox("\U0001F4E7 Email", value=True, key="notif_channel_email")
    wa_on = c3.checkbox("\U0001F4AC WhatsApp", value=False, key="notif_channel_wa")
