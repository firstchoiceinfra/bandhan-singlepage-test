    ...
    with st.expander("\u2795 Add a New Astrologer"):
        ...
        if st.button("\u2795 Add Astrologer", key="add_astro_btn"):
            if not new_astro_name or not new_astro_qr:
                ...
                db_create_astrologer(new_astro_name, new_astro_specialty, new_astro_exp, new_astro_price, qr_ref, email=new_astro_email.strip())
                st.success(f"\u2705 {new_astro_name} added. Their wallet starts at \u20b90 \u2014 recharge it below to make them Active/bookable.")
                st.rerun()
    ...
    if not astrologers_admin:
        ...
        for a in astrologers_admin:
            with st.container():
                ...
                st.markdown(f"**{a['name']}** \u2014 {a.get('specialty','')} \u2014 {status_badge}")
...
.sync-pill { display:inline-block; padding: 4px 12px; border-radius: 20px; font-size: 0.8rem; font-weight: 600; }""")
    ...
    # can't be booked until they recharge. ----
    real_astrologers = []
    if sync_ok:
        for a in db_list_astrologers():
            real_astrologers.append({
                ...
                "qr_ref": a.get("qr_ref"), "email": a.get("email", ""), "is_active": a["is_active"], "is_real": True,
            })
    demo_astrologers = [
        {"id": None, "name": "Pandit Raghunath Sharma", "specialty": "Kundli Matching & Dosha Remedies", "exp": "22 yrs experience", "rating": 4.9, "price": 499, "qr_ref": None, "is_active": True, "is_real": False},
        {"id": None, "name": "Acharya Meenakshi Joshi", "specialty": "Muhurat & Wedding Astrology", "exp": "15 yrs experience", "rating": 4.8, "price": 399, "qr_ref": None, "is_active": True, "is_real": False},
        {"id": None, "name": "Jyotishi Vikram Bhatt", "specialty": "Career & Marriage Timing", "exp": "18 yrs experience", "rating": 4.7, "price": 349, "qr_ref": None, "is_active": True, "is_real": False},
    ]
    astrologers = real_astrologers + demo_astrologers
    ...
    for idx, a in enumerate(astrologers):
        ...
        # ---- Real astrologers get paid directly by the client via their
        # own UPI QR code (there's no payment gateway wired up to split
        # this automatically) \u2014 the client confirms they've paid before
        # the booking is confirmed. Demo astrologers keep the old
        # one-click booking since there's no real money or wallet involved. ----
        if a["is_real"] and a.get("qr_ref"):
            with st.expander(f"\U0001F4B3 Pay \u20b9{a['price']} via QR to book with {a['name']}"):
                qr_bytes = media_get_bytes(a["qr_ref"])
                if qr_bytes:
                    st.image(qr_bytes, width=220, caption=f"Scan to pay \u20b9{a['price']} to {a['name']}")
                paid_confirm = st.checkbox(f"I have paid \u20b9{a['price']} to {a['name']} via the QR code above", key=f"astro_paid_{idx}")
                if bc3.button("\U0001F4C5 Confirm Booking", key=f"astro_book_{idx}", type="primary", use_container_width=True, disabled=already_booked_slot or not paid_confirm):
                    ...
                    client_name_for_astro = st.session_state.get("user_name", "A Bandhan.com client")
                    if user_email:
                    ...
                    # ---- Sent from OUR Gmail account (unavoidable \u2014 see
                    # send_generic_email's docstring on why "From" can't be
                    # the client's own address), but Reply-To is the
                    # client's real email, so the astrologer's reply goes
                    # straight to the client, not to Bandhan.com. ----
                    if a.get("email"):
                        send_generic_email(
                            a["email"], f"\U0001F4C5 New Session Booking from {client_name_for_astro} \u2014 Bandhan.com",
                            f"{client_name_for_astro} has booked a session with you for {session_date} at {session_time}.\n\n"
                            f"They have confirmed payment of \u20b9{a['price']} via your QR code.\n\n"
                            f"Client email: {user_email}\n\n"
                            f"Simply hit Reply on this email to message {client_name_for_astro} directly.",
                            reply_to=user_email,
                        )
                    st.success(f"\u2705 Session booked with {a['name']} on {session_date} at {session_time}. They've been notified of your payment.")
                    st.rerun()
        else:
    ...
    st.caption("\u26A0\uFE0F Demo mode note: astrologers you add yourself (via Company Dashboard) use their real wallet + QR payment flow above; the 3 starter astrologers shown here are illustrative examples with no real wallet.")
...
def db_create_astrologer(name, specialty, experience, price, qr_ref, email=""):
    db = get_db()
    if db is None:
        return False
    db.astrologers.insert_one({
        "name": name, "specialty": specialty, "experience": experience, "price": price,
        "qr_ref": qr_ref, "email": email, "wallet_balance": 0, "created_at": dt.datetime.utcnow(),
    })
    return True
...
.vmarket-stars { color: #D4AF37; font-size: 1.1rem; }""")
    ...
    tabs = st.tabs([f"{CATEGORY_ICONS.get(c, default_icon)} {c}" for c in categories])
...
.sync-pill { display:inline-block; padding: 4px 12px; border-radius: 20px; font-size: 0.8rem; font-weight: 600; }""")
    ...
    with tab2:
        ...
        st.caption(f"\U0001F4D0 Standard ad slot: {AD_STANDARD_SIZE}. Exactly where this appears across the rest of the app (homepage, search results, etc.) is still being finalized \u2014 for now, ads show here on this page filtered by city.")
