        ...
        with target:
            ...
            with bcol2:
                if st.button("\u2B50" if is_bookmarked else "\u2606", key=f"bm_{i}_{art['title']}", use_container_width=True, help="Save for later"):
                    ...
                    st.rerun()
            with bcol3:
                share_body = urllib.parse.quote(f"{art['title']} \u2014 read it on Bandhan.com: {art['excerpt']}")
...
.horo-card { background: white; padding: 22px; border-radius: 14px; box-shadow: 0 6px 15px rgba(0,0,0,0.06); border-top: 4px solid #8E2DE2; text-align: center; }""")
    ...
    share_body = urllib.parse.quote(f"My {sign.split()[0]} horoscope today on Bandhan.com \u2014 Love: {love_msg} Career: {career_msg} Lucky color: {lucky_color}, Lucky number: {lucky_number}")
    hs_col1, hs_col2 = st.columns(2)
    horoscope_share_subject = urllib.parse.quote("Today's Horoscope on Bandhan.com")
    hs_col1.link_button("\U0001F4E7 Share via Email", f"mailto:?subject={horoscope_share_subject}&body={share_body}", use_container_width=True)
    hs_col2.link_button("\U0001F4F1 Share on WhatsApp", f"https://wa.me/?text={share_body}", use_container_width=True)
...
.wish-box { background: linear-gradient(135deg, #FFF3E0, #FFE8CC); border: 2px dashed #D4AF37; border-radius: 10px; padding: 14px 18px; margin-top: 8px; font-style: italic; color: #7A4A00; }""")
    ...
    if "couples" not in st.session_state:
        ...
        st.session_state.couples = [
            {
                "spouse1_name": "Rahul Sharma", "spouse1_dob": today0 + datetime.timedelta(days=40), "spouse1_email": "rahul.sharma@example.com",
                "spouse2_name": "Priya Sharma", "spouse2_dob": today0 + datetime.timedelta(days=18), "spouse2_email": "priya.sharma@example.com",
        ...
        if add_couple:
            if not spouse1_name or not spouse2_name:
                ...
                st.session_state.couples.append({
                    "spouse1_name": spouse1_name, "spouse1_dob": spouse1_dob, "spouse1_email": spouse1_email,
                    "spouse2_name": spouse2_name, "spouse2_dob": spouse2_dob, "spouse2_email": spouse2_email,
    ...
    for c_idx, c in enumerate(st.session_state.couples):
        events.append({
            ...
            "recipients": [(c["spouse1_name"], c["spouse1_email"])],
        ...
        events.append({
            ...
            "recipients": [(c["spouse2_name"], c["spouse2_email"])],
        ...
        events.append({
            ...
            "recipients": [(c["spouse1_name"], c["spouse1_email"]), (c["spouse2_name"], c["spouse2_email"])],
            ...
            with rc1:
                if st.button(f"\U0001F4AC {'Send Now' if is_today else 'Send Wish Now'} \u2014 {e['person']}", key=f"wa_{e['uid']}"):
                    ...
                    for name, addr in e["recipients"]:
                        if addr:
                            success, detail = send_generic_email(addr, f"{label}: {e['person']} \U0001F495", e["wish"], reply_to=st.session_state.get("user_email"))
