        ...
        if submitted:
            ...
            if save_error == "phone_duplicate":
                ...
                if id_doc_file is not None:
                    ...
                    db_submit_id_verification(user_email, doc_ref, id_doc_type)
...
# This is upload-and-manual-review, the same pattern as photo/video
# moderation above — NOT a live government API lookup (a real UIDAI/NSDL
# KYC integration needs a licensed provider and is out of scope here).
# Once a Boss approves it, the profile gets a "Government ID Verified"
# badge wherever verification status is shown.
# =====================================================================
def db_submit_id_verification(email, doc_ref, id_type):
    db = get_db()
    if db is None:
        return False
    db.profiles.update_one({"email": email}, {"$set": {
